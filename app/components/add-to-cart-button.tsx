...
export function AddToCartButton(props: Props) {
  ...
  const existing = items.find(i => i.productId === props.productId)

  if (existing) {
    return (
      <div className="flex items-center gap-1">
        <button
          onClick={e => { e.preventDefault(); e.stopPropagation(); updateQuantity(props.productId, existing.quantity - 1) }}
